...
- Deck-builder card info blocks (Arrow Down) end with a "Style" line (e.g. "Style: Showcase Etched") read live from each tile — on both the deck side and the collection pool. Skin codes are read per row from the tile's own `MtgCardInstance.SkinCode` (deck tiles) or `_lastDisplayInfo.Skin` (pool tiles) rather than the deck-level skin dictionary, so two rows of the same card with different skins read distinctly. When no skin override is applied, the line falls back to the printing identity (set name + collector number, e.g. "Style: Dominaria 26") so two rows of the same card from different printings — different `GrpId`s — can be told apart by ear too.
- New `Shift+Enter` on a focused deck-builder card opens the card viewer popup, mirroring the sighted right-click. Works on collection cards, deck-list entries, sideboard, commanders, and read-only deck cards. The craft popup itself now leads with a "Style:" header so the user gets immediate confirmation of which printing they're spending wildcards on (different printings of the same card live in the inventory as separate `GrpId`s; the popup is single-style by design).
- New `Ctrl+Enter` on a focused pool card expands the title into one adjacent tile per available art-style or printing variant — invokes the game's preferred-printing expand chevron under the hood (`PagesMetaCardView.ExpandClicked()` → `PreferredPrintingState.ExpandCard` / `CollapseCard`), so the same fan-out a sighted player gets by clicking the tile's ribbon. Press again to collapse. Inside an active fan-out, each variant tile announces its differentiator up-front (e.g. "Sagenumwobener Durchgang, Showcase Etched" vs "Sagenumwobener Durchgang, Avatar TFTUR Extra 57") instead of repeating the bare name on every sibling; the announce reads `_lastDisplayInfo.ExpandedStyle` (`Expanded_First/Mid/Last` vs `Solo`/`Stacked`) so normal browsing stays quiet. Solo cards (no alternates) announce "No alternative styles available" instead of a misleading expanded/collapsed cue. `Shift+Enter` on the chosen variant then opens the craft popup for that exact (printing, skin). F1 help has a new "Deck builder card shortcuts" category covering both shortcuts; the deck-builder `Ctrl+F1` hint mentions them too.

Bug fixes:

- Generic mana in card costs is now read as "generic" instead of "colorless" (e.g. `{5}{U}{B}` → "5 generic, blue, black"). "Colorless" has a specific Magic meaning — `{C}` and the Colorless color enum still read as "colorless". Contributed by **@LordLuceus** (PR #90).
- Repeated mana symbols inside one brace pair no longer drop tokens. The game encodes pairs like `{B}{B}` as `{oBoB}`, and the regex consumed the leading `o` before tokenizing — so Spree's `+ {B}{B} —` line read as `+ Black —`. Now reads `+ Black, Black —`. Contributed by **@LordLuceus** (PR #90).
- Evoke and similar hybrid alternative costs are no longer dropped. The game encodes `{U/B}{U/B}` as `{o(U/B)o(U/B)}`, which the parser couldn't tokenize at all (parens weren't recognized) — so Deceit's "Evoke {U/B}{U/B}" line read as just "Evoke". Now reads "Evoke Blue or Black, Blue or Black". Braced-form parser fix contributed by **@LordLuceus** (PR #90); subsequently extended to bare-text and parameterized hanger sequences for parity.
...
- Phase-skip warning now also fires during Declare Blockers: pressing Space to confirm "no blocks" while you still have untapped creatures and nothing assigned warns once and requires a second press to confirm. The warning is gated to the opponent's turn — that's when the user is the defending player and can actually declare blockers — so attacking on your own turn no longer triggers a false-positive guard during the opponent's blocker step.
- Mana pool announcements now reflect the real amount when one ability taps for multiple mana of the same color. The game packs same-color mana into a single `MtgMana` entry with `Count=N` (Rofellos = one entry with `Count=3`); the parser was incrementing by 1 per entry, so a Rofellos tap announced "1 green" instead of "3 green". Now sums `MtgMana.Count` per color.
- Optional-trigger target-select prompts (Tinybones-style "may cast target" triggers) no longer trap the player. The browser presented a `SelectCardsMultiZone` scaffold with `AllowCancel.No` and a `DoneButton`-only provider dict; selecting the only valid target and then cancelling the cast looped back into the browser with a sticky selection that auto-resubmitted, with no escape reachable through the existing CancelButton path. Backspace now mirrors the Space-confirm guard: first press warns, second press deselects via per-card `SimulatePointerClick` and invokes `OnButtonCallback("DoneButton")`, which the game flips to the FailToFind face once the count drops to 0. The two guards mutually clear their warning bits on first-press so a Space-then-Backspace intent change can't leak armed state across a later same-key press.
- `SelectCardsMultiZone` browsers with `AllowCancel.No` and a disabled `DoneButton` (count below minimum, no zone tab to flip to) no longer trap the player in an Exit→Re-enter churn. `ClickConfirmButton` was setting `_pendingRescan = true` unconditionally up front; every provider/workflow path then failed to click anything, but the rescan still fired on next `Update` — re-announcing the browser entry and teleporting focus back to card 1, drowning the failure feedback. The fall-through path now clears `_pendingRescan` (no click happened, no scaffold-reuse to track), and the previously generic "No confirm button found" announcement is replaced with the already-translated "No target selected" when the provider dict reports `DoneButton` disabled — so the player hears the actionable game-state cause instead of a mod-error string.
- Card cost announcements now reflect active cost reductions instead of always reading the printed cost. With a Warden of Evos Isle on the battlefield, a second Warden in hand now reads `1 generic, blue` (matching the in-hand tile) instead of `2 generic, blue` (the printed cost). The mod now reads the live cast-action cost (`Action.ManaCost` on the per-card `Actions` list, converted via `GreClient.CardData.ManaUtilities.ConvertManaCostsToList`) — the same source MTGA's own card-tile renderer uses — and falls back to `PrintedCastingCost` when no cast action is available. Library / collection / deck-builder views (no live instance, so no actions) still read the printed cost as before. Contributed by **@LordLuceus** (PR #91).
- Stack additions now announce every new item, not just the visible top. When the opponent responds to your spell with their own (so two things land on the stack near-simultaneously — their response plus, often, one of their triggered abilities that fires off the same cost or trigger), the response cast used to go silent: the mod inspected only the top CDC, which was usually the trigger, and never spoke the response itself. The mod now tracks announced stack items by `MtgCardInstance.InstanceId` and announces every newly-added CDC on each Stack zone-count update, clearing the tracker when the stack drains.
...
- Damage assignment browser is no longer hijacked by the generic SpinnerNavigator. Both navigators were activating on `SpinnerAnimated` widgets, but SpinnerNavigator ran first and stole input — announcing blockers as `Card #483` (its battlefield-only resolver missed cards moved into the browser), reporting "Verteile 0" (no parent SpinnerGroup → max=0), and submitting via `PromptButton_Primary` instead of the browser's `DoneAction` so confirming the default lethal assignment did nothing. SpinnerNavigator now yields when an AssignDamage browser is active, letting `BrowserNavigator.AssignDamage` handle it cleanly (proper card names, total damage, lethal status, Space → DoneAction, Backspace → UndoAction).
- Mutate Yes/No prompt (e.g. Huntmaster Liger asking "mutate onto X?") no longer locks the player out. The scaffold `BrowserScaffold_YesNo_Mutate_…` was being collapsed to plain `YesNo` by `ExtractBrowserTypeFromScaffold`, losing the variant: Tab cycled only the mutation-pile cards (never reaching the Yes/No buttons), Space fell through to the duel-level `PromptButton_Primary` (the pass button — wrong target, browser stayed stuck), the entry hint announced the literal locale key `BrowserHint`, and the friendly name read "Choose yes or no" instead of "Mutate choice". `BrowserDetector` now flags the Mutate variant on `BrowserInfo`, and `BrowserNavigator` mirrors the OptionalAction flow for it: Tab cycles cards then buttons, Space routes through the provider's logical `DoneButton` (resolves to `2Button_Right` = Yes), Backspace through `CancelButton` (= No), with a proper `BrowserHint_Mutate` hint translated for all 12 locales.
- Yes/No browser prompts now read the actual question instead of just "Choose yes or no". Ancestral Recall's "Are You Sure?" popup (and any other button-only browser like Optional-action / Mutate prompts) reads `Do you want to target your opponent with Ancestral Recall? Choose yes or no` — the scaffold's subheader text is pulled via the existing `ExtractBrowserHeaderText` helper and prepended to the type label. Contributed by **@LordLuceus** (PR #93).
- Battlefield creature P/T now announces live damage as a separate piece after counters, matching how the game shows the red number next to the toughness for sighted players. A 3/3 that took 2 damage reads `3/3, 2 damage` (or with counters: `4/4, 1 +1/+1, 2 damage`); the printed/effect-modified toughness stays intact so it's clear how the value was built. Reads `MtgCardInstance.Damage` off the model — 0 when there's no live instance, so deck builder / collection / library reveals stay unaffected. Feature originally contributed by **@LordLuceus** (PR #92, as a toughness adjustment); reworked to surface damage as its own line so the announcement mirrors the game's visual layout.
- Opponent's hand (Shift+C) no longer lists cards that have left the hand. Cards the opponent has cast (their CDC lingers under the OpponentHand holder with `IsDisplayedFaceUp=true` even after moving to graveyard / exile / battlefield) used to stay in the revealed-cards list, producing a "Opponent's hand, 2 cards. … 1 of 3" mismatch where the displayed count and the navigable list disagreed and a card the opponent had already cast still appeared as if it were in their hand. The OpponentHand list now applies the same `IsRealHandCard` model-zone check the hand count already used, so list and count agree. Contributed by **@LordLuceus** (PR #95).
- Tab navigation no longer drops cards with affordable actions. MTGA's visual HotHighlight isn't reliably refreshed in transitional states — both on castable hand cards (notably immediately after a land play, where the battlefield creature's activated-ability highlight stays but the player's still-castable hand cards lose their visual cue) and on the player's own battlefield permanents with activated abilities (e.g. a creature with `{T}: do X` whose glow didn't refresh after an opposing spell resolved into the graveyard). The mod's scan only matched GameObjects whose names contained the literal substring `"HotHighlight"`, so Tab would cycle whichever item happened to still have its glow. Both hand and player-side battlefield are now supplemented with a lookup into MTGA's own `ActionsAvailableWorkflow`: the workflow tracks a `GreInteraction` per available action with a pre-computed `CanAffordToCast` bool (set when the interaction is constructed from `Action.CanAffordToCast()`), and we include any card that has a playable interaction — any `Cast*` variant (split halves, adventure, MDFC, prototype, Rooms, Omens), plain `Activate` (cycling, channel, foretell-the-card, plot-the-card, ninjutsu, on-battlefield activated abilities, planeswalker abilities), or non-payment `Special`/`SpecialTurnFaceUp` (conspire, monarch, dungeon advance, morph/manifest flip) — that the workflow itself considers affordable. Mana abilities (`ActivateMana`), land drops (`Play*`), opponent permanents, and payment subactions are filtered out so Tab still only cycles meaningful actions. No mod-side affordability heuristics — the game answers the question. Castable-hand foundation contributed by **@LordLuceus** (PR #96); subsequently widened to `Activate`/`Special` action types and extended to player-side battlefield permanents.

## v1.1

Duel:

- New optional "Battlefield stacking" setting (F2, off by default) groups identical battlefield cards into one navigable entry ("N Cardname"), mirroring the game's visual stacking. Land Summary keeps the true count. When targeting splits a stack, focus auto-advances to the next unselected copy; announcements include selection state.
- New Ctrl+Enter on a stacked card toggles the whole stack at once — assigns/removes all copies as attackers or blockers in combat, and works in any workflow that accepts stack clicks (targets, group/selection). Falls back to plain Enter for non-stacked cards.
- New J hotkey jumps between attachments and targets — cycles through attachment parent, attached children, targets, and things targeting the focused card.

Bug fixes:

- Faction-based sealed events (e.g. Strixhaven Mystical Archive Sealed) now work end-to-end. The V2 `FactionalizedEventTemplate` was previously unknown to the mod, leaving the entire event screen with no title, no event context, and an unreadable wall of faction descriptions. Now: the screen is recognized as an event page; the description splits into a generic intro plus one info block per faction (language-agnostic via shared loc keys); faction tiles announce which faction is selected and rescan on switch.
- Event-page payment buttons now include the currency name (e.g. "2000 Gems" / "2000 Gold") — the game conveys currency only via sprite icon, invisible to screen readers.
- Mastery reward levels no longer double-announce quantity (e.g. "600x 600 Gems" → "600 Gems") when the localized text already embeds the count.
- Opponent's exile (Shift+X) and command zone (Shift+W) now find their content via per-player sub-holders. Revealed opponent-hand cards and castable-from-exile cards (Plot/Foretell) are navigable under Shift+C.
- I (extended card info) shows the mana cost of castable linked faces — Adventure, Prepared Spell, Split, MDFC back, Room, Omen, Prototype.
- Browser confirm with a non-zero selection (e.g. Starfield Shepherd's library search) now actually submits when the game collapses to a single button.
- Hand size and index count only real hand cards; flashback/foretell/commander cards no longer inflate the count. Applies to C and Shift+C.
- Combat phase no longer re-announces "All Attack" as if it were an action after deselecting the last attacker.
- Deck-screen filter checkboxes labeled correctly again (no leaked search placeholder / event-pool tooltips).
- Game-loading status steps are spoken once each instead of being silenced.
- System message popups during game loading (e.g. "Arena must be restarted") are now announced.
- Friends panel reliably shows incoming/outgoing request groups when invites exist.
- Pack opening: revealing an off-screen card no longer announces its name twice (inline announcement + post-rescan label-change announcement collided).
- Spanish localization: pack type names and "Timer" label translated.
- French localization: 24 strings with double-encoded accents fixed (Land Summary, Full Control, Phase Stop, Loyalty, etc.).

Documentation:

- README translated into French, Italian, Japanese, Korean, Polish, Brazilian Portuguese, Russian, Simplified Chinese, and Traditional Chinese — joining the existing German and Spanish translations.
...
- Fixed phase-skip warning also firing when confirming a browser with Space — the browser closed mid-press, so by the time the guard's hook ran the modal bypass no longer applied; guard now tracks whether a modal was active at any point during the current Space hold
